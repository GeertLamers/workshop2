...
public class AddressDAOImpl implements AddressDAO {
		...
		try (Connection conn = DataSource.getConnection();
			...
			try (ResultSet rs2 = stmt.getGeneratedKeys();){
				if (rs2.next()) {
					...
					stmt.executeUpdate();
					newAddressId = Long.valueOf(rs.getInt(1));
	...
	public List<Address> findAllAddresses() {
		...
		try (Connection conn = DataSource.getConnection();
			...
			while(rs.next()) {
				...
				address.setAddressId(Long.valueOf(rs.getInt(1)));
				...
				customer.setCustomerId(Long.valueOf(rs.getInt(8)));
	...
	public Address findAddressById(Long addressId) {
		...
		try (Connection conn = DataSource.getConnection();
			...
			if(rs.next()) {
				foundAddress.setAddressId(Long.valueOf(rs.getInt(1)));
